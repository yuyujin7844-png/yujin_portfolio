...
export default function TicketReservationModal({ open, onClose, defaultTicket }) {
  ...
  // 모달이 열릴 때 기본 티켓 반영
  const resolvedForm = { ...form, ticketType: form.ticketType || defaultTicket || 'red' };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!resolvedForm.name.trim() || !resolvedForm.phone.trim()) {
    ...
    setError('');
    try {
      await submitTicketReservation(resolvedForm);
      setSuccess(true);
      setForm(EMPTY);
    } catch {
      ...
      <DialogContent sx={{ pt: 3 }}>
        {success ? (
          ...
          <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2.5, mt: 1 }}>
            ...
            <TextField name="name" label="이름 *" value={resolvedForm.name} onChange={handleChange} fullWidth sx={FIELD_SX} />
            <TextField name="phone" label="연락처 *" value={resolvedForm.phone} onChange={handleChange} fullWidth sx={FIELD_SX} placeholder="010-0000-0000" />
            <TextField name="email" label="이메일 (선택)" type="email" value={resolvedForm.email} onChange={handleChange} fullWidth sx={FIELD_SX} />
